...
public class WebListParser {
  ...
  public static Set<Title> parse(String url)
      throws InvalidListTypeException, InvalidListPageException {
    try {
  ...
  private static Set<Title> parseGoodreads(String url) throws IOException {
    ...
    do {
      ...
      nextButton = document.getElementsByClass("next_page").last();
      if (nextButton != null) {
        url = nextButton.attr("abs:href");
      }

      Elements titles = document.getElementsByClass("field title");
      resultSet.addAll(
          titles.stream().map(title -> title.getElementsByAttribute("title").first())
              .map(link -> new Title(link.ownText(), link.attr("abs:href")))
  ...
  private static Set<Title> parseKinopoisk(String url) throws IOException {
    ...
    while (url != null) {
      ...
      if (document.title().equals("Вы не робот?") || document.title().equals("Oops!")) {
        return resultSet;
      }
  ...
  private static Set<Title> parseImdb(String url) throws IOException {
    ...
    do {
      document = Jsoup.connect(url + page).maxBodySize(0).get();
      nextButton = document.getElementsByAttributeValue("aria-label", "Next").first();

      String jsonTitles = document.getElementsByAttributeValue("type", "application/ld+json")
          .first().data();
      jsonTitles = jsonTitles.substring(jsonTitles.indexOf('['), jsonTitles.indexOf(']') + 1);

      ObjectMapper objectMapper = new ObjectMapper();
      List<ImdbListItem<Title>> wrappedTitles = objectMapper.readValue(jsonTitles,
          new TypeReference<List<ImdbListItem<Title>>>() {
          });

      for (ImdbListItem<Title> listItem : wrappedTitles) {
        resultSet.add(listItem.getItem());
      }

      page++;
    } while (nextButton != null && !nextButton.hasAttr("disabled"));
  ...
  private static Set<Title> parseMyanimelist(String url) throws IOException {
    ...
    url += "/load.json?offset=";
    Set<Title> resultSet = new HashSet<>();
    String jsonTitles = ".";

    int offset = 0;

    while (!jsonTitles.equals("[]")) {
      jsonTitles = Jsoup.connect(url + offset).ignoreContentType(true)
          .maxBodySize(0).execute().body();
      ObjectMapper objectMapper = new ObjectMapper();

      resultSet.addAll(objectMapper.readValue(jsonTitles,
  ...
  public static void save(Set<Title> titles, String filename) throws IOException {
    ...
    objectMapper.writerWithDefaultPrettyPrinter()
        .writeValue(Files.newOutputStream(Path.of(filename)), titles);
  }

  public static Set<Title> mapYoutubeVideoToTitle(
      Set<YoutubeHistoryParser.YoutubeVideo> youtubeVideos) {
  ...
  @JsonIgnoreProperties(value = {"status", "score", "tags", "is_rewatching", "num_watched_episodes",
      "created_at", "updated_at", "anime_title_eng", "anime_num_episodes", "anime_airing_status",
      "anime_id", "anime_studios", "anime_licensors", "anime_season", "anime_total_members",
      "anime_total_scores", "anime_score_diff", "anime_popularity", "anime_score_val",
      "has_episode_video", "has_promotion_video", "has_video", "genres", "demographics",
      "title_localized", "video_url", "anime_image_path", "is_added_to_list",
      "anime_media_type_string", "anime_mpaa_rating_string", "start_date_string",
      "finish_date_string", "anime_start_date_string", "anime_end_date_string", "days_string",
      "storage_string", "priority_string", "notes", "editable_notes", "id", "is_rereading",
      "num_read_chapters", "num_read_volumes", "manga_english", "manga_num_chapters",
      "manga_num_volumes", "manga_publishing_status", "manga_id", "manga_magazines",
      "manga_total_members", "manga_total_scores", "manga_score_val", "manga_image_path",
      "manga_media_type_string", "manga_end_date_string", "retail_string",
      "manga_start_date_string", "@type", "description", "image", "aggregateRating",
      "contentRating", "genre", "duration", "alternateName"
  })
  public static class Title {

    @JsonProperty("title")
    @JsonAlias({"anime_title", "manga_title", "name"})
    private String title;
    @JsonProperty("link")
    @JsonAlias({"anime_url", "manga_url", "url"})
    private String link;
  ...
  private static class ImdbListItem<T> {
